    ...
    async def key(self, ctx, oper, key=None, *args):
        ...
        try:
            ...
            if oper == 'add' and value:
                ...
                await ctx.invoke(self.bot.get_command('key'), query=f'add {testkey} abc123')
                await ctx.send(f"Testing `key get {testkey}`")
                await ctx.invoke(self.bot.get_command('key'), query=f'get {testkey}')
                await ctx.send(f"Testing `key edit {testkey} xyz789`")
                await ctx.invoke(self.bot.get_command('key'), query=f'edit {testkey} xyz789')
                await ctx.send(f"Testing `key get {testkey}`")
                await ctx.invoke(self.bot.get_command('key'), query=f'get {testkey}')
                await ctx.send(f"Testing `key delete {testkey}`")
                await ctx.invoke(self.bot.get_command('key'), query=f'delete {testkey}')
                ...
                await ctx.invoke(self.bot.get_command('key'), query=f'add {testkey}')
                await ctx.send(f"Testing `key add {testkey} multi word value`")
                await ctx.invoke(self.bot.get_command('key'), query=f'add {testkey} multi word value')
                await ctx.send(f"Testing `key get {testkey}`")
                await ctx.invoke(self.bot.get_command('key'), query=f'get {testkey}')
                await ctx.send(f"Testing `key add {testkey} mno456` (already exists, should fail)")
                await ctx.invoke(self.bot.get_command('key'), query=f'add {testkey} mno456')
